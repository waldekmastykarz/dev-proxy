...
if (DevProxyCommand.IsDetachedMode && !DevProxyCommand.IsInternalDaemon)
{
...
static async Task<int> StartDetachedProcessAsync(string[] args)
{
    ...
    // Replace --detached/-d with --_internal-daemon
    var daemonArgs = args
        .Where(a => a is not "--detached" and not "-d")
    ...
    try
    {
        ...
        await Console.Error.WriteLineAsync($"Check the log file: {DevProxyCommand.DetachedLogFilePath}");
        return 1;
    }
    catch (Exception ex)
    {
